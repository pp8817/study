...
public class OrderRepository {
    ...
    /**
     ...
     distinct를 사용한 이유는 일대다(ToMany) 조인이 있으므로 데이터베이스 row가 증가한다.
     ...
     컬렉션 페치 조인은 1개만 사용할 수 있다. 컬렉션 둘 이상에 페치 조인을 사용하면 안된다. 데이터가 부정합하게 조회된다.
     */
    public List<Order> findAllWithItem() {
        return em.createQuery(
                    //fetch join: 지연 로딩(Lazy)를 강제로 즉시 로딩(EAGER)으로 변경해서 한방 쿼리로 변경
                "select distinct o from Order o " +
                        "join fetch o.member m " +
                        "join fetch o.delivery d " +
                        "join fetch o.orderItems oi " +
                        "join fetch oi.item i", Order.class) //JPA에서 엔티티를 조회
                .getResultList();
    }
